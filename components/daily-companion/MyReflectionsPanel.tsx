...
export function MyReflectionsPanel({ userId }: MyReflectionsPanelProps) {
  ...
  return (
    <div className="space-y-3">
      <h3 className="text-lg font-semibold text-[#2a2318]">Your Recent Reflections</h3>
      {reflections.map((reflection) => (
        <Card key={reflection.id} className="bg-[#f8f6f2]">
          <div className="space-y-2">
            <p className="text-xs text-[#8fa878] font-semibold uppercase tracking-wide">
              {new Date(reflection.created_at).toLocaleDateString()}
            </p>
            <p className="text-xs text-[#6b5f52] italic mb-3">"{reflection.prompt_text}"</p>
            ...
            {editingId === reflection.id ? (
              <div className="space-y-3">
                <textarea
                  ...
                  className="w-full px-3 py-2 border border-[#e8ddd2] rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#d4a574] bg-white"
                  rows={3}
                />
              ...
              <div className="space-y-3">
                <p className="text-sm text-[#2a2318] leading-relaxed">{reflection.response}</p>
                <div className="flex gap-2 pt-2 border-t border-[#e8ddd2]">
                  <button
                    ...
                    className="text-xs text-[#d4a574] hover:text-[#c09560] font-medium"
                  >
                    Edit
                  </button>
                  <button
                    onClick={() => handleDelete(reflection.id)}
                    className="text-xs text-[#a0968a] hover:text-[#6b5f52] font-medium"
                  >
                    Delete
                  </button>
                </div>
