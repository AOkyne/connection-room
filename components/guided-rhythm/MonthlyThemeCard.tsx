...
export function MonthlyThemeCard({ month, onSelectRitualOption }: MonthlyThemeCardProps) {
  return (
    <Card className="bg-gradient-to-br from-[#f3ede5] to-[#f8f6f2] border-l-4 border-[#d4a348]">
      <div className="space-y-4">
        {/* Month Number and Title */}
        <div className="flex items-baseline gap-3">
          <span className="text-4xl font-light text-[#d4a348]">
        ...
        <div className="pt-4 border-t border-[#e8ddd2]">
          <p className="text-[#1a0f0a] leading-relaxed">{month.monthlyTheme}</p>
        </div>
        ...
        {month.ritual && (
          <div className="bg-[#f8f6f2] rounded-lg p-4">
            ...
            <p className="text-xs text-[#1a0f0a] mb-3">{month.ritual.description}</p>
              ...
              <div className="flex flex-wrap gap-3">
                {month.ritual.options.map((option, idx) => (
                  <button
                    ...
                    className="px-3 py-2 text-xs text-[#1a0f0a] rounded-lg border border-[#d4a348] hover:bg-[#f3ede5] transition-colors"
                  >
                    {option}
                  </button>
                ))}
