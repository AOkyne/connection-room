...
export default function AppHome() {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Welcome Section */}
      <div className="space-y-2">
        <h1 className="text-4xl text-[#1a0f0a]">Welcome, {profile.displayName}</h1>
      ...
      <div className="mb-8">
        {primaryAction.onClick ? (
          ...
          >
            <Card className="bg-gradient-to-br from-[#f3ede5] to-[#fffbf7] border-[#d4a348] hover:border-[#c99563] transition-colors hover:shadow-md cursor-pointer h-full">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  <h2 className="text-2xl font-semibold text-[#1a0f0a]">{primaryAction.title}</h2>
                  <p className="text-[#1a0f0a] mt-2">{primaryAction.description}</p>
                  <div className="mt-4">
                    <div className="inline-flex items-center gap-2 px-4 py-2 bg-[#8b6f47] text-white rounded-lg font-medium hover:bg-[#7d6245] transition-colors">
                ...
                {primaryAction.icon && (() => {
                  ...
                  return <Icon size={40} className="text-[#d4a348] flex-shrink-0" />;
                })()}
              </div>
            </Card>
          </button>
      ...
      <div className="space-y-6">
        ...
        {joinedSpacesCount > 0 && (
          <div>
            <h3 className="text-sm font-medium text-[#c97a2a] mb-3 uppercase tracking-wide">Your Spaces</h3>
        ...
        <div id="todays-reflection">
          <h3 className="text-sm font-medium text-[#c97a2a] mb-3 uppercase tracking-wide">Today's Reflection</h3>
          <Card className="bg-[#fffbf7] border-[#e8ddd2]">
            <div className="space-y-3">
              <p className="text-[#1a0f0a] italic text-base">"{todaysPrompt}"</p>
              <p className="text-xs text-[#a0704a]">A sentence or two is enough.</p>
              <button
                onClick={() => {
                  setPromptResponse("");
                  const commonsSpace = spaces.find(s => s.id === "commons");
                ...
                className="text-sm font-medium text-[#8b6f47] hover:text-[#7d6245] transition-colors inline-flex items-center gap-1"
              >
                Respond →
              </button>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <h3 className="text-sm font-medium text-[#c97a2a] mb-3 uppercase tracking-wide">Connections</h3>
            <WaysToConnectCard />
          </div>
          <div>
            <h3 className="text-sm font-medium text-[#c97a2a] mb-3 uppercase tracking-wide">From the Room</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <h3 className="text-sm font-medium text-[#c97a2a] mb-3 uppercase tracking-wide">Your Journey</h3>
            <FirstWeekDashboardCard />
          </div>
          <div>
            <h3 className="text-sm font-medium text-[#c97a2a] mb-3 uppercase tracking-wide">Guided Path</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {upcomingEvents.length > 0 && (
            <div>
              <h3 className="text-sm font-medium text-[#c97a2a] mb-3 uppercase tracking-wide">Upcoming</h3>
              <Card className="bg-[#fffbf7] border-[#e8ddd2]">
                <div className="space-y-2">
                  <p className="font-medium text-[#1a0f0a] text-sm">{upcomingEvents[0].title}</p>
                  <p className="text-xs text-[#a0704a]">
                    {upcomingEvents[0].date.toLocaleDateString()} at {upcomingEvents[0].time}
                  </p>
                  <Link href="/app/events">
                    <button className="text-sm font-medium text-[#8b6f47] hover:text-[#7d6245] transition-colors">
                      All Events →
                    </button>
                  </Link>
                </div>
          ...
          <div>
            <h3 className="text-sm font-medium text-[#c97a2a] mb-3 uppercase tracking-wide">Discover</h3>
            {profile?.quizResult && profile.quizResult !== "I have not taken the quiz yet" ? (
              <Card className="bg-gradient-to-br from-[#f3ede5] to-[#fffbf7] border-[#c97a2a]">
                <div className="space-y-2">
                  <p className="text-xs text-[#c97a2a] font-medium uppercase tracking-wide">Your Connection Profile</p>
                  <p className="text-sm font-semibold text-[#1a0f0a]">{profile.quizResult}</p>
                  <p className="text-xs text-[#1a0f0a]">This reveals how you tend to connect with others.</p>
                  <div className="flex gap-2 pt-1">
                    ...
                    <Link href="/app/quizzes">
                      <button className="text-sm font-medium text-[#a0704a] hover:text-[#1a0f0a] transition-colors">
                        Retake
                      </button>
                    </Link>
                  </div>
              ...
              <Card className="bg-[#fffbf7] border-[#e8ddd2]">
                <div className="space-y-2">
                  <p className="text-xs text-[#1a0f0a]">Learn about your intimacy patterns with our curated quizzes.</p>
                  <Link href="/app/quizzes">
                    <button className="text-sm font-medium text-[#8b6f47] hover:text-[#7d6245] transition-colors">
                      Take a Quiz →
                    </button>
          ...
          {badges.length > 0 && (
            <div>
              <h3 className="text-sm font-medium text-[#c97a2a] mb-3 uppercase tracking-wide">Achievements</h3>
              <Card className="bg-[#fffbf7] border-[#e8ddd2]">
                <div className="space-y-2">
                  {badges.slice(0, 1).map((badge) => {
                    ...
                    return (
                      <div key={badge.id} className="flex items-start gap-2">
                        <BadgeIcon size={16} className="text-[#d4a348] flex-shrink-0 mt-1" />
                        <div className="text-xs">
                          <p className="font-medium text-[#1a0f0a]">{badge.name}</p>
                          <p className="text-[#a0704a]">{badge.description}</p>
                        </div>
                      </div>
                    );
                  })}
                  {badges.length > 1 && (
                    <p className="text-xs text-[#a0704a] pt-1">{badges.length - 1} more →</p>
                  )}
                </div>
              </Card>
            </div>
          ...
          {offers.length > 0 && (
            <div>
              <h3 className="text-sm font-medium text-[#c97a2a] mb-3 uppercase tracking-wide">For You</h3>
              <Card className="bg-[#fffbf7] border-[#e8ddd2]">
                <div className="space-y-2">
                  {offers.slice(0, 1).map((offer) => (
                    <div key={offer.id} className="space-y-2">
                      <p className="font-medium text-[#1a0f0a] text-sm">{offer.title}</p>
                      <p className="text-xs text-[#a0704a]">{offer.description}</p>
      ...
      {/* Info Banner */}
      <div className="bg-[#f3ede5] border border-[#e8ddd2] rounded-lg p-4 text-sm text-[#1a0f0a]">
        <p className="font-medium text-[#1a0f0a] mb-2 flex items-center gap-2"><IconDemo size={16} /> Demo Mode</p>
        <p>
      ...
      >
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-2xl font-semibold text-[#1a0f0a]">Respond to Prompt</h2>
          <button
            onClick={() => dialogRef.current?.close()}
            className="text-[#a0704a] hover:text-[#1a0f0a] text-2xl font-bold leading-none"
          >
            ×
          </button>
        </div>
        ...
        <div className="space-y-4 max-h-[70vh] overflow-y-auto">
          <div className="bg-[#f3ede5] rounded-lg p-4">
            <p className="text-[#1a0f0a] italic">"{todaysPrompt}"</p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              className="w-full px-3 py-2 border border-[#e8e3db] rounded-lg focus:outline-none focus:border-[#d4a348] text-[#1a0f0a] bg-white"
              rows={4}
            />
            <p className="text-xs text-[#c97a2a]">A sentence or two is enough.</p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-[#c97a2a]">Everyone in this space will see your response and can reply.</p>
          </div>
          ...
          <div className="flex gap-3 pt-4">
            <button
              type="button"
              onClick={() => dialogRef.current?.close()}
              className="flex-1 font-medium rounded-lg transition-all duration-150 focus:outline-none border-2 border-[#d4a348] text-[#8b6f47] hover:bg-[#faf7f2] px-4 py-2 text-base"
            >
              Cancel
            </button>
            <button
              ...
              className="flex-1 font-medium rounded-lg transition-all duration-150 focus:outline-none bg-[#8b6f47] text-white hover:bg-[#7d6245] active:bg-[#6a523a] disabled:opacity-50 disabled:cursor-not-allowed px-4 py-2 text-base"
            >
