...
export default function SpaceMembersPage() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#faf7f2] flex items-center justify-center">
        <p className="text-[#a0968a]">Loading members...</p>
      </div>
    );
  }
  ...
  if (!space) {
    return (
      <div className="min-h-screen bg-[#faf7f2] flex items-center justify-center">
        <p className="text-[#6b5f52]">Space not found</p>
      </div>
    );
  }
        ...
        <div className="max-w-6xl mx-auto px-4 py-4 sm:px-6">
          <div className="flex items-center gap-4">
            <Link
              ...
              className="text-[#d4a574] hover:text-[#c9956d]"
            >
              Back
            </Link>
            <div>
              <h1 className="text-2xl font-semibold text-[#2a2318]">
                {space.name} - Members
              </h1>
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 sm:px-6">
        {members.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-[#6b5f52]">No members have joined this space yet.</p>
          </div>
        ) : (
                ...
                <div className="bg-white rounded-lg overflow-hidden hover:shadow-lg transition-shadow cursor-pointer group">
                  ...
                  <div className="relative aspect-square overflow-hidden bg-[#f0e8e0]">
                    ...
                    {member.is_demo_profile && (
                      <div className="absolute top-2 right-2 bg-[#e8ddd2] text-[#a0968a] text-xs font-normal px-2 py-0.5 rounded">
                        Sample
                      </div>
                    )}
                  </div>
                  ...
                  <div className="p-4 space-y-3">
                    <div>
                      <h3 className="font-semibold text-[#2a2318] group-hover:text-[#d4a574] transition-colors">
                        {member.displayName}
                      </h3>
                      {member.pronouns && (
                        <p className="text-sm text-[#a0968a]">
                    ...
                    {member.interests && member.interests.length > 0 && (
                      <div className="flex flex-wrap gap-1">
                        {member.interests.slice(0, 2).map((interest) => (
                          <span
                            key={interest}
                            className="inline-block text-xs bg-[#f3ede5] text-[#6b5f52] px-2 py-1 rounded"
                          >
                            {interest}
                          </span>
                        ))}
                        {member.interests.length > 2 && (
                          <span className="inline-block text-xs text-[#a0968a]">
